...
def run_estimator(simConst=SimulationConst(), estConst=EstimatorConst(), doplot=True, seed=0):
    ...
    for k in range(1,N):
        ...
        posterior = Estimator(posterior, sense[k,:], input[k-1,:], copy.deepcopy(estConst), km[k])
    ...
    """ Plotting """
    if doplot:
        ani = Animation(N_particles, x_r, y_r, phi, rho, kappa, state, sense, simConst, fps=120)
