...
class Animation(animation.TimedAnimation):
    def __init__(self, N_particles, x_r, y_r, phi, rho, kappa, state, sense, simConst: SimulationConst, fps=120):
        ...
        self.contour[0,1] += self.state[0,3]
        self.contour[1,1] += self.state[0,3]
        self.contour[7,0] += self.state[0,4]
        self.contour[8,0] += self.state[0,4]
        ...
        self.particle_walls_01 = Line2D([], [], color='red', alpha=0.75)
        self.particle_walls_78 = Line2D([], [], color='red', alpha=0.75)
        
        ax.add_line(self.contour_plot)
        ax.add_line(self.robot_area)
        ax.add_line(self.distance_line)
        ax.add_line(self.particle_circles)
        ax.add_line(self.particle_walls_01)
        ax.add_line(self.particle_walls_78)
    ...
    def _draw_frame(self, framedata):
        ...
        min_rho = np.min(self.rho[k,:]) + self.simConst_contour[0,1]
        max_rho = np.max(self.rho[k,:]) + self.simConst_contour[1,1]
        self.particle_walls_01.set(ydata=np.tile([min_rho, max_rho], 100))

        sort_kappa = np.sort(self.kappa[k,:]) + self.simConst_contour[7,0]
        self.particle_walls_78.set(xdata=np.repeat(sort_kappa, 2))
        ...
        self._drawn_artists = [self.robot_area, self.particle_circles, self.particle_walls_78, self.distance_line]
    ...
    def _init_draw(self):
        ...
        self.particle_walls_01.set(xdata=np.linspace(self.contour[0,0], self.contour[1,0], 200, endpoint=True))
        self.particle_walls_78.set(ydata=np.tile([self.contour[7,1], self.contour[8,1]], self.N_particles))
